...
const EditFoodFormModal = ({ open, children, onClose, food }) => {
    ...
    const [inputs, setInputs] = useState({ title: food?.title, description: food?.description, city: food?.city, geolocation: food?.address })
            ...
            .then(response => {
                if (response.status === 200) {
                       const res = posts.map((el) => {
                        if (el.id === food.id) {
                            return { ...el, title, description, city }
                        } else {
                            return el
                        }})
                    console.log('res--', res)
            ...
            <div className="two-div main-agileinfo" style={MODAL_STYLES}>
                ...
                <div className="agileits-top formdesign">
                    ...
                    <form className="" onSubmit={handleSubmit}>
                        ...
                        {/*{err}*/}
                        <input className="text inputformdecor" type="text" name="title" placeholder="Название" required="" value={inputs.title} onChange={handleChange} />
